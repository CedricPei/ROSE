...
from prompts.prompt_pr_wogt import system_prompt_pr_wogt, user_prompt_pr_wogt


class PR_WOGT:
    """Prover-Refuter combined in one step, without ground truth"""
    
    def __init__(self, model: str = None, output_dir: str = "output"):
        self.model = model
        self.output_dir = output_dir
    
    def call(self, question, pred_sql: str, pred_result):
        """Validate predicted SQL without gold standard"""
        try:
            ...
            user_content = user_prompt_pr_wogt.format(
            ...
            response = client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": system_prompt_pr_wogt},
            ...
            output_file = os.path.join(self.output_dir, "pr_wogt_output.json")
            save_json(output_data, output_file, append=True)

            return result.get("verdict", None), result.get("reason", "")

        except Exception as e:
            print(f"PR_WOGT error: {e}")
            return None, f"Error: {e}"


def _process_question(question, pr_wogt, output_dir):
    ...
    score = 0.0
    pr_wogt_verdict = None

    if pred_res is None:
        return 
    if pred_res is False:
        score = 0.0
        write_result_to_file(question, pred_sql, score, pr_wogt_verdict, None, output_dir)
        return 
    
    pr_wogt_verdict, _ = pr_wogt.call(question, pred_sql, pred_res)
    if pr_wogt_verdict is None:
        return 
    score = 1.0 if pr_wogt_verdict else 0.0
    write_result_to_file(question, pred_sql, score, pr_wogt_verdict, None, output_dir)
    return 


def main():
    parser = argparse.ArgumentParser(description="PR_WOGT (Prover-Refuter combined without ground truth) ablation experiment")
    ...
    output_dir = f"output/test/gemini-ablation/pr-wogt"
    os.makedirs(output_dir, exist_ok=True)

    pr_wogt = PR_WOGT(model=reasoning_model, output_dir=output_dir)
    ...
    def worker(idx):
        ...
        for q in slice_questions:
            _process_question(q, pr_wogt, output_dir)
            progress.update(1)
        return []
